...
import { createQuizSchema } from '@/lib/quizzes/schemas';
...
export const GET = route<{ id: string }>(async (_req: NextRequest, { params }) => {
  ...
  return NextResponse.json(
    await svc.getForLesson(params.id, { userId: user.id, role: user.role }),
  );
});

export const POST = route<{ id: string }>(async (req: NextRequest, { params }) => {
  const user = await requireSession();
  const body = await readJson(req, createQuizSchema);
  const svc = new QuizzesService(prisma);
  return NextResponse.json(
